...
## Ближайшая реализация: контент‑план и автогенерация статей (документация)

Ниже зафиксирована спецификация новой функциональности. Реализация в коде будет добавляться поэтапно.

- Контент‑план и темы с контролем очередности:
  - Модель `ContentPlan` (привязка к `Site`) и связанная модель `Topic` с полем `position` для строгого порядка.
  - Статусы тем: `queued`, `generating`, `done`, `failed`.
- Менеджмент‑команда автодозарядки тем:
  - При запуске проверяет текущее количество тем в активном плане; если меньше целевого лимита (пока 10), делает запрос во внешний сервис по заданному промпту и описанию сайта и добавляет недостающие темы.
  - Формат ответа сервиса: JSON со списком тем. См. `docs/commands.md`.
- Модели `ContentGenerator` и `ContentGeneratorLog`:
  - `ContentGenerator` — служебная точка входа (пока без полей), содержит метод для запроса генерации статьи по очередной теме.
  - `ContentGeneratorLog` — журнал запросов/ответов и ошибок по темам/генерации.
- Генерация статьи:
  - Метод `ContentGenerator` берёт следующую тему из плана, формирует запрос (описание сайта + промпт), отправляет во внешний сервис и по завершении создаёт модель `Article` в проекте.

Подробности:
- База данных и модели — см. `docs/db/README.md`.
- Команды и форматы API — см. `docs/commands.md`.
- Решения и обоснования — см. `docs/architecture/decisions.md`.